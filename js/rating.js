(function () {
  ...
  var STORAGE_KEY = "instructcompare_rating";
  var CLIENT_ID_KEY = "instructcompare_clientid";
  var SEED = [4, 5, 4, 3, 5, 4, 4, 5, 3, 4];

  var buttons = document.getElementById("rating-buttons");
  var avgEl = document.getElementById("rating-avg-value");
  var totalEl = document.getElementById("rating-total");
  var yoursEl = document.getElementById("rating-yours");
  ...
  function renderUI(avg, total, user) {
    ...
    var btns = buttons && buttons.querySelectorAll(".rating-btn");
    if (btns) {
  ...
  // --- Firestore ---
  var statsRef = db && db.collection("stats").doc("rating");
  var votesRef = function () { return db && db.collection("votes").doc(getClientId()); };
  ...
  function onVoteFirebase(val) {
    ...
    var stats = db.collection("stats").doc("rating");
    var vote = db.collection("votes").doc(clientId);
